...
function addProduct(srcImage, name, id, price) {
  const htmlElement =
    ...
    srcImage +
    '" alt="product-img height="200" width="200" "/> <div class="card-body text-center"> <h4>' +
    name +
    "</h4> <h5>(" +
    id +
    ')</h5><p class="mute">' +
    price +
    '</p><a href="#" class="btn btn-success btn-sm"> details</a> <button class="btn btn-success btn-sm add-button"> <i class="fa fa-plus" aria-hidden="true"></i> </button> </div></div>';
...
function search(searchTag) {
  ...
  for (i = 0; i < currentList.length; i++) {
    if (
      currentList[i].name.toUpperCase().includes(temp) ||
